...
pub fn start_virtual_machine() {
    thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            let mut ticker = interval(tDuration::from_millis(10000));
            loop {
                ticker.tick().await;
                let (actual_height, _actual_hash, _) = get_latest_block_info();
				let actual_vm_height;
				if actual_height > vm_height {
					actual_vm_height = actual_height - UNLOCK_OFFSET - UNLOCK_OFFSET;
				} else {
					actual_vm_height = 0;
				}
								...
								match dtx {
									Ok(tx) => {
										...
										if input_hex != "0x" && address != CONTRACT_CREATOR {
											...
											if method == "0x40c10f19" {
												let info_key = format!("{}:type", address);
												let maybe_info_value = vmdb.get(&info_key);
												let mut creator = String::new();
												
												print_log_message("minting".to_string(), 1);

												if let Ok(Some(value_bytes)) = maybe_info_value {
													if let Ok(value_str) = std::str::from_utf8(&value_bytes) {
														...
														if parts.len() >= 2 {
															...
															continue;
															print_log_message("No 2 parts".to_string(), 1);
														}
													} else {
														print_log_message("Error converting creator string".to_string(), 1);
														continue;
													}
												} else {
												...
												if creator.to_lowercase() == sender_address.to_lowercase()
												{
													...
													let hex_vm_sender = format!("{:0>64}", vm_sender);
													let sender_balance_key = format!("{}:{}{}", address, ERC20_FN_CHECK_BALANCE, hex_vm_sender);
													let big_int_amount = BigUint::parse_bytes(params[0].as_bytes(), 16).expect("Invalid hex string");
													let mut sender_balance: BigUint;;
													match vmdb.get(&sender_balance_key) {
														Ok(Some(value)) => {
															let hex_str = String::from_utf8_lossy(value.as_ref());
											...
											if input_hex != "0x" {
												...
												if method == ERC20_MINTABLE_CREATE || method == ERC20_NON_MINTABLE_CREATE {
													if params.len() == 4 {
														...
														let info_value = format!("{}:{}", method, sender_address);
